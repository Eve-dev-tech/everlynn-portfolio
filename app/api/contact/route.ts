...
export async function POST(req: Request) {
  try {
    const { challenge, triedAlready, whyNow, email } = await req.json()

    if (!challenge || !triedAlready || !whyNow || !email) {
        ...
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0D0D1A;color:#F0F0F0;border-radius:12px;overflow:hidden;">
          ...
          <div style="padding:32px;">
            <table style="width:100%;border-collapse:collapse;margin-bottom:24px;">
              <tr><td style="padding:8px 0;color:#8892A4;font-size:13px;width:140px;">Client Email</td><td style="padding:8px 0;font-size:15px;"><a href="mailto:${email}" style="color:#00B5AD;">${email}</a></td></tr>
            </table>
            <div style="margin-bottom:20px;padding:20px;background:#16213E;border-radius:10px;border-left:3px solid #00B5AD;">
              <p style="margin:0 0 8px;color:#8892A4;font-size:12px;text-transform:uppercase;letter-spacing:0.1em;">1. Biggest challenge right now</p>
              <p style="margin:0;font-size:15px;line-height:1.75;">${challenge}</p>
            </div>
            <div style="margin-bottom:20px;padding:20px;background:#16213E;border-radius:10px;border-left:3px solid #00B5AD;">
              <p style="margin:0 0 8px;color:#8892A4;font-size:12px;text-transform:uppercase;letter-spacing:0.1em;">2. What they've tried</p>
              <p style="margin:0;font-size:15px;line-height:1.75;">${triedAlready}</p>
            </div>
            <div style="margin-bottom:24px;padding:20px;background:#16213E;border-radius:10px;border-left:3px solid #00B5AD;">
              <p style="margin:0 0 8px;color:#8892A4;font-size:12px;text-transform:uppercase;letter-spacing:0.1em;">3. Why now</p>
              <p style="margin:0;font-size:15px;line-height:1.75;">${whyNow}</p>
            </div>
            <div style="margin-top:24px;">
              <a href="mailto:${email}" style="display:inline-block;background:#00B5AD;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:600;font-size:14px;">Reply to ${email}</a>
            </div>
          </div>
        </div>
      `,
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#0D0D1A;color:#F0F0F0;border-radius:12px;overflow:hidden;">
          ...
          <div style="padding:32px;">
            <p style="font-size:16px;line-height:1.8;margin-bottom:20px;">Hi there,</p>
